      ...
      <select name="mancc" class="form-control browser-default custom-select">
        ...
        while ($row2 = mysqli_fetch_array($rs2)) { ?>
          <option <?php if ($row2['MaNCC'] === $kq_sua['MaNCC']) {
            echo "selected";
          } ?> value="<?php echo $row2['MaNCC']; ?>">
            <?php echo $row2['MaNCC'] . ' - ' . $row2['TenNCC']; ?>
          </option>
        <?php } ?>
      </select>
    </div>
    <div class="form-group col-3"><label>Đơn Giá</label><input type="text" class="form-control" name="dongia"
        value="<?php echo $kq_sua['DonGia']; ?>"></div>
    <!-- <div class="form-group col-2"><label>Số Lượng</label><input type="text" class="form-control" name="soluong" id="soluong" value="<?php echo $kq_sua['SoLuong']; ?>"
                ></div> -->
    <div class="form-group col-6"><label>Ảnh Nền</label> <input type="file" class="form-control" name="anhnen"> </div>
    <div class="form-group col-6"><label>Ảnh sản phẩm</label> <input type="file" class="form-control" name="anhsp[]"
        multiple></div>
    <div class="form-group col-12"><label>Mô Tả</label><textarea class="form-control" name="mota"
        style="min-height: 100px;"><?php echo $kq_sua['MoTa']; ?></textarea> </div>
        ...
        while ($kq_size = mysqli_fetch_array($rs_size)) {
          ...
          <div class="custom-checkbox custom-control col-1">
            <input type="checkbox" class="custom-control-input" id="<?php echo $kq_size['MaSize']; ?>" name="size[]"
              value="<?php echo $kq_size['MaSize']; ?>" <?php while ($kq_size2 = mysqli_fetch_array($rs_size2)) {
                   if ($kq_size['MaSize'] === $kq_size2['MaSize']) {
                     echo "checked";
                   }
                 } ?>>
            <label class="custom-control-label" for="<?php echo $kq_size['MaSize']; ?>">
              <h5>
                <?php echo $kq_size['MaSize']; ?>
              </h5>
            </label>
          </div>
          <?php
    ...
    <div class="form-group col-sm-12  border ">
      ...
      <div class="btn-group col-12 row">
        ...
        while ($kq_mau = mysqli_fetch_array($rs_mau)) {
          $mm = $kq_mau['MaMau'];
          $sql_mau2 = "select DISTINCT MaMau from chitietsanpham where MaSP=$masp and MaMau='$mm'";
          $rs_mau2 = mysqli_query($conn, $sql_mau2); ?>
          <div class=" custom-checkbox custom-control col-2 ">
            <input type="checkbox" class="custom-control-input" id="<?php echo $kq_mau['MaMau']; ?>" name="mau[]"
              value="<?php echo $kq_mau['MaMau']; ?>" <?php while ($kq_mau2 = mysqli_fetch_array($rs_mau2)) {
                   if ($kq_mau['MaMau'] === $kq_mau2['MaMau']) {
                     echo "checked";
                   }
                 } ?>>
            <label class="custom-control-label" for="<?php echo $kq_mau['MaMau']; ?>">
              <h5>
                <?php echo $kq_mau['MaMau']; ?>
              </h5>
            </label>
          </div>
        <?php }
